...
def read_aeris(file):
    ...
    aeris['Time_UTC'] = pd.to_datetime(aeris['Time Stamp'], errors='coerce', format='%m/%d/%Y %H:%M:%S.%f')
...
def read_met(file):
    ...
    cols = [
        'PC', 'UTC hhmmss', 'UTC Year', 'UTC Month', 'UTC Day', 'Latitude (DD.ddd +N)',
        'Longitude (DDD.ddd -W)', 'GPS Quality', 'Altitude (m)', 'Air Temperature (C)',
        'RH(%)', 'Dew Point (C)', 'Wind Direction (Deg True)', 'Wind Direction (Deg Mag)',
        'Wind Speed (m/s)', 'Pressure (bar)', 'PCB1 Temperature (C)', 'PCB2 Temperature (C)',
        'Supply Voltage (VDC)', 'Heading(deg)', 'GPSCorWindDirTrue (deg)', 'GPSCorWindDirMag (deg)',
        'GPSCorWindSpeed (kts)', 'GPSCorWindSpeed (m/s)', 'VehicleSpeed'
    ]
    met = pd.read_csv(file, header=3, names=cols, index_col=False,
                      skipinitialspace=True, on_bad_lines='skip',
                      dtype={'UTC hhmmss': str})
    met_time = dict(year=met['UTC Year'], month=met['UTC Month'], day=met['UTC Day'],
    ...
    met = met[['Latitude (DD.ddd +N)', 'Longitude (DDD.ddd -W)', 'GPS Quality',
                'Altitude (m)', 'Air Temperature (C)', 'RH(%)', 'Dew Point (C)',
                'Pressure (bar)', 'GPSCorWindDirTrue (deg)', 'GPSCorWindSpeed (kts)',
                'VehicleSpeed']]
    met = met.rename(columns={'Latitude (DD.ddd +N)': 'latitude', 'Longitude (DDD.ddd -W)': 'longitude'})
    met['filename'] = file
    return met
...
def merge_aeris_met(aeris, met):
    ...
    met['u_wind'], met['v_wind'] = wind_components(met['GPSCorWindSpeed (kts)'], met['GPSCorWindDirTrue (deg)'])
...
def enhanced_R_and_ratio(data, window='30s'):
    data.rename(columns={'R': 'R_aeris',
                         'C2/C1': 'C2C1_aeris'}, inplace=True)
...
def wyomingMap(data, param, ax=None, extent=None, tiler=None, tiler_zoom=12, cmap='YlOrRd', figsize=(16,10), plot_winds=True, windskip=40, title=None,
               **kwargs):
