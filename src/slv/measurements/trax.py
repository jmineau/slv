...
def load_lines(meters=False) -> gpd.GeoDataFrame:
    lines = gpd.read_file(
        f"{GROUP_DIR}/spatial/transportation/light_rail/UTA_TRAX.geojson"
    )
    if meters:
...
def load_obs(
    ...
    lgr_manual = uataq.read_data(
        "trx01",
        instruments="lgr_ugga_manual_cal",
        lvl="qaqc",
        time_range=time_range,
        num_processes=num_processes,
    )["lgr_ugga_manual_cal"]
    ...
    lgr = lgr[lgr.QAQC_Flag.isin(valid_flags) & (lgr.ID_CH4 == -10) & lgr.CH4.notnull()]
    lgr = lgr.drop(columns=["QAQC_Flag", "ID_CH4"])
    lgr_manual = lgr_manual[
        lgr_manual.QAQC_Flag.isin(valid_flags)
    ...
    gps = gpd.sjoin(gps, lines_buff, how="inner", predicate="within").drop(
        columns=["index_right"]
    )
    ...
    data = uataq.sites.MobileSite.merge_gps(
        lgr.rename_axis("Pi_Time", axis=0), gps, on="Pi_Time"
    ).drop(columns=["Pi_Time"])
