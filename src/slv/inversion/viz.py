...
def plot_sites(ax, sites, site_config, color='black'):
    for site in sites:
        ax.scatter(site_config.at[site, 'longitude'],
                   site_config.at[site, 'latitude'],
                   transform=PC, label=site.upper(), c=color,
                   marker='o', s=100, edgecolor='white', linewidth=1.5)

    return ax


def plot_grid(grid, extent, tiler, zoom,
              add_sites=True, sites=None, site_config=None, site_color='black',
              add_point_sources={
                'landfill': 'yellow',
                'refinery': 'cyan',
                'powerplant': 'orange',
...
def plot_concentrations(obs):
    ...
    ax.set(title='Hourly Averaged Valid CH$_4$ Observations',
        xlabel='Time [UTC]',
        ylabel='CH$_4$ [ppm]')
    fig.autofmt_xdate()
...
def plot_fluxes(problem, tiler, zoom,
                add_sites=True, sites=None, site_config=None, site_color='black',
...
def plot_fluxes_by_timestep(problem, tiler, zoom,
                        add_sites=True, sites=None, site_config=None, site_color='black',
                        add_point_sources=None):
    facet = problem.posterior_fluxes.to_xarray().astyle(float).plot(col='time', col_wrap=8,
                                                                    subplot_kws={'projection': tiler.crs},
                                                                    transform=PC, alpha=0.6, cmap='coolwarm'
                                                                    )
...
def plot_total_fluxes_over_time(*total_fluxes: pd.Series):
    ...
    ax.set(title='Total Emissions for Inversion Domain',
           xlabel='Time',
