...
from slv.measurements.daq import load_picarro_g2307, sites as daq_sites
from slv.meteorology.pcaps import filter_pcap_events


def load_site_data(sites, time_range, num_processes=1, filter_pcaps=True) -> pd.DataFrame:
    ...
    if 'wbb' in sites:
        wbb = uataq.get_obs('wbb', 'ch4', time_range=time_range, num_processes=num_processes)[pollutant_col]
        data_dict['wbb'] = wbb
    ...
    for site in sites:
        if site in daq_sites:
            ...
            daq_df = daq_df[conditions].set_index('Time_UTC').sort_index().loc[time_range]
...
def get_slv_observations(sites: list[str], 
    time_range: tuple, 
    num_processes: int = 1, 
