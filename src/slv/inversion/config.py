...
from slv.domain import XMIN, XMAX, YMIN, YMAX, UTC_OFFSET
...
class InversionConfig:
    ...
    afternoon_hours_local: list[int] = field(default_factory=lambda: [12, 13, 14, 15, 16]) 
    ...
    aggregate_obs: bool | str = False  # Whether to aggregate obs space ('1d' for daily, '12h' for 12-hourly, etc.)
    ...
    # --- Jacobian ---
    stilt_path: str = '/uufs/chpc.utah.edu/common/home/u6036966/wkspace/methane/SLV/stilt/wbb'
    ...
    mdm_components: list[MDMComponent] = field(default_factory=lambda: [
    ...
    def map_extent(self):
        ...
        buffer = 0.05
        return (self.xmin - buffer, self.xmax + buffer, self.ymin - buffer, self.ymax + buffer)
    ...
    def grid(self):
        ...
        grid = generate_regular_grid(xmin=self.xmin, xmax=self.xmax, dx=self.dx,
                                    ymin=self.ymin, ymax=self.ymax, dy=self.dy,
    ...
    def grid_coords(self):
        return pd.MultiIndex.from_product([self.grid['lon'].values,
                                           self.grid['lat'].values]).to_list()
